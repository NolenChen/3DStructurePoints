...
def train_one_epoch(model, optimizer, data_loader, lr_scheduler, bnm_scheduler, current_iter, criterions, num_of_trans,
    ...
    for batch in data_loader:
        ...
        if num_of_trans > 0:
            transed_batch_points, rot_mats, trans_func_list = data_utils.AddPCATransformsToBatchPoints(batch_points,
                                                                                                       num_of_trans=num_of_trans)
            transed_batch_points = transed_batch_points.view(batch_size * num_of_trans, transed_batch_points.shape[2],
                                                             transed_batch_points.shape[3])
        ...
        if num_of_trans > 0:
            transed_batch_points = transed_batch_points.view(num_of_trans, batch_size, transed_batch_points.shape[1],
                                                             transed_batch_points.shape[2])
            transed_structure_points = structure_points_all[1:(1 + num_of_trans)]

        if num_of_trans > 0:
            loss = criterions['ComputeLoss3d'](batch_points, structure_points, transed_batch_points,
                                               transed_structure_points, trans_func_list)
        else:
...
def train(cmd_args):
    ...
    train_set = bhcp_dataloader.bhcp_dataloader(cmd_args.data_dir, cmd_args.category, transforms=trans,
    ...
    for epoch_i in range(start_epoch, cmd_args.max_epochs):
        ...
        train_loss, train_cd_loss, train_consistent_loss, iters = train_one_epoch(model, optimizer, train_loader,
